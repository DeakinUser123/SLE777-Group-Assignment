...
head(sorted_gene_expression,10) #Show the first 10 genes in the sorted table
```
...
growth_data_raw <- read.csv(URL) #Read the csv from the URL 
colnames(growth_data_raw) #Get the column names of "growth_data_raw"
```
...
northeast_results_task7 # Show the results table for northeast
southwest_results_task7 # Show the results table for southwest
```
...
#Display the mean growth results for each site
...
```{r}
#Create the data set for this task, done via subtracting the 2020 data from the 2010 data for growth over 10 years and put it into a table
...
t_test_result <- t.test(northeast_data_task10$Growth_10yr, southwest_data_task10$Growth_10yr) # Using a t-test 
print(t_test_result) # Printing the results 
```
